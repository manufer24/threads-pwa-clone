    ...
    <div id="CreatePost" class="fixed z-50 bottom-0 h-full w-full overflow-hidden">
        <div class="bg-black h-full w-full text-white overflow-auto">
            ...
            <button 
                ...
                :class="isLoading ? 'text-gray-600' : 'text-black'"
            >
                <div v-if="!isLoading" class="rounded-full bg-gray-600 w-16 mx-auto">Post</div>
...
const createPost = async () => {
    ...
    if (errorOut) {
        console.log(`this is the first error: ${errorOut}`)
        return errorOut
    }
    ...
    try {
        ...
        console.log(`you are not logged ${error}`)
        isLoading.value = false
    }
